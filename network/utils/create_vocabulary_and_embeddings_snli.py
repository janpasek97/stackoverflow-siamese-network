"""
Create embedding matrix and dictionary from pre-trained tf.hub Word2Vec model for SNLI dataset specifically
"""
import argparse
import csv
import pickle
...
from network.utils.train_utils import EMBEDDING_MATRIX_SNLI_FN, IDX_TO_WORD_SNLI_FN, WORD_TO_IDX_SNLI_FN, SNLI_DS_DIR

EMBEDDING_DIM = 250
VOCAB_SET_EXPORT_FN = "network/assets/snli_w2v/vocabulary_set.dat"
CLEANED_VOCAB_EXPORT_FN = "network/assets/snli_w2v/vocabulary_set_clean.dat"
snli_data, snli_info = tfds.load("snli", split="train", with_info=True, data_dir=SNLI_DS_DIR)


def get_vocabulary_set(text_cleaner):
    """
    ...
    for i, example in enumerate(snli_data):
        first_post_text = text_cleaner.clean_text(example['hypothesis'].numpy().decode("utf-8"))
        first_post_tokens = tokenizer.tokenize(first_post_text)
        vocab_set.update(first_post_tokens)

        second_post_text = text_cleaner.clean_text(example['premise'].numpy().decode("utf-8"))
...
def create_vocab_set_and_embeddings(args):
    ...
    if args.vocab_set:
        ...
        vocabulary = sorted(get_vocabulary_set(TextCleaner()))
    ...
    if args.dictionaries:
        ...
        with open(WORD_TO_IDX_SNLI_FN, "wb") as word_to_idx_file:
            pickle.dump(word_dict, word_to_idx_file)

        with open(IDX_TO_WORD_SNLI_FN, "wb") as idx_to_word_file:
    ...
    if args.embedding:
        ...
        try:
            with open(IDX_TO_WORD_SNLI_FN, "rb") as idx_to_word_file:
        ...
        np.save(file=EMBEDDING_MATRIX_SNLI_FN, arr=embedding_matrix)
        run_time = time.time() - start_time
        print(f"    {int(run_time / 60)} min, {int(run_time % 60)} s")
