"""
Implementation of dataset generator that loads the items from csv file directly
"""
from network.utils.data_generator_base import DataGeneratorBase


class FileDataGenerator(DataGeneratorBase):
    """
    CSV file dataset generator. Child class of DataGeneratorBase
    """
    def __init__(self, csv_text_file, word2idx, idx2word, max_sentence_length=250, pad_sentences=True,
        ...
        """
        super(FileDataGenerator, self).__init__(csv_text_file, word2idx, idx2word, max_sentence_length, pad_sentences,
    ...
    def get_post_content_and_label(self, csv_row):
        """
        Get dataset item from the the parsed CSV directly
        :param csv_row: row of the dataset pre-processed text or code
        :return: dataset item
        """
        first_post_content = csv_row[0]
        second_post_content = csv_row[1]
        label = csv_row[2]
