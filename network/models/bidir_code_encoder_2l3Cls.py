...
class BiDirCodeEncoder2L3Cls(tf.keras.Model):
    def __init__(self, embedding_weights_pretrained, hparams, num_reserved_words, code_embeddings_weights_pretrained):
        ...
        # output layers
        self.dense1 = tf.keras.layers.Dense(128, activation="relu", kernel_regularizer=tf.keras.regularizers.l2(
            hparams[constants.HP_L2_REGULARIZATION]))
        self.out = tf.keras.layers.Dense(3, activation="softmax", use_bias=False,
    ...
    def __call__(self, x, training=False, **kwargs):
        ...
        text_x1, text_x2 = self.dropout1(text_x1), self.dropout1(text_x2)
        text_x1, text_x2 = self.bi_dir_lstm1(text_x1), self.bi_dir_lstm1(text_x2)
        text_x1, text_x2 = self.dropout2(text_x1), self.dropout2(text_x2)
        ...
        code_x1, code_x2 = self.dropout1(code_x1), self.dropout1(code_x2)
        code_x1, code_x2 = self.code_bidir_lstm1(code_x1), self.code_bidir_lstm1(code_x2)
        code_x1, code_x2 = self.dropout2(code_x1), self.dropout2(code_x2)
